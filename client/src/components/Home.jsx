...
export default function Home() {
  ...
  allSongs.forEach((song) => {
    ...
    const albumName = rawName.replace(/[_-]/g, ' ').replace(/\d+$/, '').trim().split(' ').slice(0, 3).join(' ').trim();
  ...
  return (
    <div className="min-h-screen bg-black text-white relative">
      ...
      {showLibrary && (
        <FavsPanel isOpen={showLibrary} onClose={() => setShowLibrary(false)} defaultTab={libraryTab} />
      )}
      ...
      {/*
        pt-24 clears the fixed header (~72px)
        pb-32 gives space above the fixed bottom player bar (~80px)
      */}
      <div className="pt-24 md:pt-28 px-6 md:px-12 pb-32 relative z-10">
          ...
          <div className="mb-12">
            ...
            <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
              {filteredSongs.map((song, i) => (
                <SongCard key={song._id || i} song={song}
        ...
        <section ref={newRef} className="mb-14 scroll-mt-28">
          ...
          {newSongs.length === 0 ? <EmptyState text="No new releases yet." />
        ...
        <section ref={moodRef} className="mb-14 scroll-mt-28">
          <div className="flex items-center gap-4 mb-5">
            ...
            <button
              ...
              style={{ padding: '7px 16px', borderRadius: '99px', border: '1px solid #06b6d433', background: '#06b6d410', color: '#06b6d4', fontSize: '12px', fontWeight: '600', cursor: 'pointer', flexShrink: 0, transition: 'background 0.15s' }}
          ...
          <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
            {MOODS.map((mood) => {
              ...
              return (
                <motion.div key={mood} whileHover={{ scale: 1.04 }}
        ...
        <section ref={languageRef} className="mb-14 scroll-mt-28">
          ...
          <div className="flex gap-3 mb-5">
            {['', 'Hindi', 'English'].map((lang) => (
              <button key={lang || 'all'} onClick={() => setSelectedLanguage(lang)}
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 hover:scale-105
                  ${selectedLanguage === lang ? 'bg-cyan-500 text-black font-bold' : 'bg-gray-900/60 text-gray-400 border border-gray-700 hover:border-cyan-700'}`}
              >
                {lang || 'All'}
              </button>
            ))}
          </div>
          {languageSongs.length === 0 ? <EmptyState text="No songs in this language yet." />
        ...
        <section ref={artistRef} className="mb-14 scroll-mt-28">
          ...
          {Object.keys(artistMap).length === 0 ? <EmptyState text="No artist songs yet." /> : (
            <div className="flex gap-5 overflow-x-auto pb-3 scrollbar-hide">
              {Object.entries(artistMap).map(([artist, artistSongs]) => (
                <motion.div key={artist} whileHover={{ scale: 1.05 }}
        ...
        <section ref={albumRef} className="mb-14 scroll-mt-28">
          ...
          {albums.length === 0 ? <EmptyState text="No albums yet." /> : (
            <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
              {albums.map(([albumName, albumSongs]) => (
                <motion.div key={albumName} whileHover={{ scale: 1.04 }}
                      ...
                      <div className="w-full h-full grid grid-cols-2 gap-0.5">
                        {albumSongs.slice(0, 4).map((s, i) => (
                          <div key={i} className="overflow-hidden">
                            {s.imageURL ? <img src={s.imageURL} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-indigo-900/50" />}
                          </div>
                        ))}
                      </div>
                    ) : (
        ...
        <section ref={playlistRef} className="mb-14 scroll-mt-28">
          ...
          {!playlists || playlists.length === 0 ? <EmptyState text="No playlists yet. Click the playlist icon on any song." /> : (
            <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
              {playlists.map((pl) => (
                <motion.div key={pl._id} whileHover={{ scale: 1.04 }}
        ...
        <section ref={songsRef} className="scroll-mt-28">
          ...
          {allSongs.length === 0 ? <EmptyState text="No songs yet." />
  ...
  return (
    ...
    >
      <div className="w-full aspect-square bg-zinc-800 flex items-center justify-center overflow-hidden relative" onClick={onPlay}>
        {song.imageURL ? <img src={song.imageURL} alt={song.title} className="w-full h-full object-cover" /> : <span className="text-5xl opacity-20">♪</span>}
